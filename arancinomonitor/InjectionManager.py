...
from arancinomonitor.LoadInjector import SpinInjection, MemoryUsageInjection
...
def get_all_injectors(inj_duration):
    ...
    """
    inj_list = [SpinInjection(duration_ms=inj_duration),
...
class InjectionManager:
    ...
    def __init__(self, inj_duration: float = 1, error_rate: float = 0.02):
        """
        Constructor
        """
    ...
    def campaign_body(self, cycle_ms: int = 1000, cycles: int = 100, verbose: bool = True):
        ...
        for cycle_id in range(0, cycles):
            start_ms = current_ms()
            if (random.randint(0, 999) / 999.0) <= self.error_rate:
    ...
    def collect_injections(self, verbose=True) -> list:
        ...
        if self.is_campaign_running():
            print("Injection campaign is still running")
            return []
        else:
